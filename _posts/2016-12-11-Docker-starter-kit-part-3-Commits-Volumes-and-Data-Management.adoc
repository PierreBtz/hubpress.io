...
Under windows again, you'll need to specify the path manually as the PWD variable does not exist.
----
...
##### Mounting on an existing directory
If a container contains data in the mount point, then this data is kept, and the host directory content overlays the container's mount point.
